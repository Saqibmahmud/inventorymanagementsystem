...
<div class="card">
    <div class="card-header">
        ...
        <a href="{{ route('sales.create') }}" class="btn btn-info float-right">+ New Sale</a>
    </div>
    <div class="card-body">
        <x-session-message/>


<p class="text-center text-lg font-black" id="report_header">Pending Sales Orders</p>
...
function loadSales(status='pending',append=false){
...
$.ajax({
    ...
    $('#sales-data').append(Html);
          ...
          $('#report_header').text(status.charAt(0).toUpperCase() + status.slice(1) + " Sales Orders");
    },
    error:function(xhr){
alert('And Error occured'+xhr.statusText );
    }
});
